...
class generic<T, ndim_, 1> {
    public:
         ...
         */
        static CCTK_HOST CCTK_DEVICE inline int multiindex(int const a) {
            return a;
        }
...
class generic<T, ndim_, 2> {
    public:
        ...
        //! Access the components of a tensor using the natural indices
        CCTK_HOST CCTK_DEVICE inline T const & operator()(int const a, int const b) const {
            return this->operator[](multiindex(a,b));
        }
...
class generic<T, ndim_, 3> {
    public:
         ...
         *  The data is stored in a row major format
         */
        static CCTK_HOST CCTK_DEVICE inline int multiindex(int const a, int const b, int const c) {
        ...
        //! Access the components of a tensor using the natural indices
        CCTK_HOST CCTK_DEVICE inline T & operator()(int const a, int const b, int const c) {
...
class symmetric2<T, ndim_, 3> {
    public:
        ...
        //! Access the components of a tensor using the natural indices
        CCTK_HOST CCTK_DEVICE inline T const & operator()(int const a, int const b, int const c) const {
            return this->operator[](multiindex(a,b,c));
        }
        ...
        //! Access the components of a tensor using the natural indices
        CCTK_HOST CCTK_DEVICE inline T const & at(int const a, int const b, int const c) const {
            return this->operator[](multiindex(a,b,c));
        }
    private:
        T m_data[ndof];
...
CCTK_HOST CCTK_DEVICE inline void contract(
...
template<typename T, int N>
CCTK_HOST CCTK_DEVICE inline void contract2(
        symmetric2<T, N, 2> const & mat,
        symmetric2<T, N, 2> const & A,
        symmetric2<T, N, 2> * B) {
    for (int a = 0; a < N; ++a)
...
template<typename T, int N>
CCTK_HOST CCTK_DEVICE inline T dot(
        generic<T, N, 1> const & va,
...
class metric<3>: public symmetric2<CCTK_REAL, 3, 2> {
    public:
        ...
        CCTK_HOST CCTK_DEVICE inline CCTK_REAL det() const {
            return nuX_Utils::metric::spatial_det(
                (*this)(0,0),
                (*this)(0,1),
                (*this)(0,2),
                (*this)(1,1),
...
class inv_metric<3>: public symmetric2<CCTK_REAL, 3, 2> {
    public:
        ...
        CCTK_HOST CCTK_DEVICE inline void from_metric(
                CCTK_REAL const gxx,
                CCTK_REAL const gxy,
                CCTK_REAL const gxz,
                CCTK_REAL const gyy,
                CCTK_REAL const gyz,
                CCTK_REAL const gzz) {
            CCTK_REAL const det = nuX_Utils::metric::spatial_det(
                    gxx, gxy, gxz, gyy, gyz, gzz);
            CCTK_REAL uxx, uxy, uxz, uyy, uyz, uzz;
            nuX_Utils::metric::spatial_inv(
                det,
                gxx, gxy, gxz, gyy, gyz, gzz,
        ...
        //! Constructs the inverse metric from the metric and the spatial det
        CCTK_HOST CCTK_DEVICE inline void from_metric_det(metric<3> const & g, CCTK_REAL const det) {
            CCTK_REAL uxx, uxy, uxz, uyy, uyz, uzz;
            nuX_Utils::metric::spatial_inv(
                det,
                g[0], g[1], g[2], g[3], g[4], g[5],
...
class metric<4>: public symmetric2<CCTK_REAL, 4, 2> {
    public:
        ...
        CCTK_HOST CCTK_DEVICE inline void from_adm(
                CCTK_REAL const alp,
                ...
                CCTK_REAL const gzz) {
            CCTK_REAL g[16];
            nuX_Utils::metric::spacetime(alp, betax, betay, betaz, gxx, gxy, gxz,
                    gyy, gyz, gzz, &g[0]);
            for(int a = 0; a < 4; ++a)
            for(int b = a; b < 4; ++b) {
                this->at(a, b) = g[4*a + b];
...
class inv_metric<4>: public symmetric2<CCTK_REAL, 4, 2> {
    public:
        ...
        CCTK_HOST CCTK_DEVICE inline void from_adm(
                ...
                CCTK_REAL const gzz) {
            CCTK_REAL u[16];
            nuX_Utils::metric::spacetime_upper(alp, betax, betay,
...
class fluid_velocity_field_const {
    public:
        ...
        CCTK_HOST CCTK_DEVICE inline void get(ptrdiff_t const ijk, generic<CCTK_REAL, 4, 1> * u) const {
            nuX_Utils::valencia::uvel(
                m_data[0][ijk], m_data[1][ijk],
        ...
        CCTK_HOST CCTK_DEVICE inline void get(ptrdiff_t const ijk, generic<CCTK_REAL, 3, 1> * v) const {
            (*v)(0) = m_data[5][ijk];
            (*v)(1) = m_data[6][ijk];
            (*v)(2) = m_data[7][ijk];
        }
...
class slicing_geometry_const {
    public:
        ...
        CCTK_HOST CCTK_DEVICE inline void get_normal(ptrdiff_t const ijk,
                generic<CCTK_REAL, 4, 1> * n) const {
            nuX_Utils::metric::normal(
                m_data[0][ijk], m_data[1][ijk], m_data[2][ijk], m_data[3][ijk],
                n->data()
            );
        }
        //! Get the normal one-form to the spacelike hypersurface
        CCTK_HOST CCTK_DEVICE inline void get_normal_form(ptrdiff_t const ijk,
        ...
        CCTK_HOST CCTK_DEVICE inline void get_shift_vec(ptrdiff_t const ijk,
        ...
        CCTK_HOST CCTK_DEVICE inline CCTK_REAL const * get_shift_comp(int const a) const {
            return m_data[1 + a];
        }
        //! Get a component of the spatial metric on the entire grid
        CCTK_HOST CCTK_DEVICE inline CCTK_REAL const * get_space_metric_comp(int const a, int const b) const {
            return m_data[4 + metric<3>::multiindex(a,b)];
        }
        //! Get a component of the extrinsic curvature on the entire grid
        CCTK_HOST CCTK_DEVICE inline CCTK_REAL const * get_extr_curv_comp(int const a, int const b) const {
            return m_data[10 + symmetric2<CCTK_REAL,3,2>::multiindex(a,b)];
        }

        //! Get the spatial metric at a given location
        CCTK_HOST CCTK_DEVICE inline void get_metric(ptrdiff_t const ijk, metric<3> * g) const {
            (*g)(0,0) = m_data[4][ijk];
            (*g)(0,1) = m_data[5][ijk];
            (*g)(0,2) = m_data[6][ijk];
            (*g)(1,1) = m_data[7][ijk];
        ...
        CCTK_HOST CCTK_DEVICE inline void get_space_proj(ptrdiff_t const ijk,
        ...
        CCTK_HOST CCTK_DEVICE inline void get_metric(ptrdiff_t const ijk, metric<4> * g) const {
        ...
        CCTK_HOST CCTK_DEVICE inline void get_extr_curv(ptrdiff_t const ijk,
