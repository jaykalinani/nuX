...
static inline closure_t pick_closure_fun(const char *name) {
  if (CCTK_Equals(name, "Eddington")) return eddington;
  if (CCTK_Equals(name, "Kershaw"))   return kershaw;
  if (CCTK_Equals(name, "Minerbo"))   return minerbo;
  if (CCTK_Equals(name, "thin"))      return thin;
  char msg[BUFSIZ];
...
//  CCTK_REAL const dt = CCTK_DELTA_TIME / static_cast<CCTK_REAL>(*TimeIntegratorStage);
//  --(*TimeIntegratorStage);
  ...
  tensor::slicing_geometry_const geom(
      ...
      volform);
  tensor::fluid_velocity_field_const fidu(
      alp, betax, betay, betaz,
      fidu_w_lorentz, fidu_velx, fidu_vely, fidu_velz);
  ...
  grid.loop_int_device<1, 1, 1>(
	...
	// Explicit sources (THC EXPL)
        ...
        CCTK_REAL J = rJ[i4D];
        CCTK_REAL const Gamma = compute_Gamma(fidu_w_lorentz[ijk], v_u, J, E, F_d, rad_E_floor, rad_eps);
        ...
        calc_closure(cctkGH, i, j, k, ig,
                     closure_fun,
                     g_dd, g_uu, n_d, fidu_w_lorentz[ijk],
                     u_u, v_d, proj_ud, Estar, Fstar_d,
        ...
        for (int a = 1; a < 4; ++a) Hnew_d(a) = Hstar_d(a) / (1 + dtau * khat);
        Hnew_d(0) = 0.0;
        for (int a = 1; a < 4; ++a) Hnew_d(0) -= Hnew_d(a) * (u_u(a) / u_u(0));

        // Update T^{μν} pieces
        CCTK_REAL const H2 = tensor::dot(g_uu, Hnew_d, Hnew_d);

# if (NUX_M1_SRC_METHOD == NUX_M1_SRC_BOOST) || (NUX_M1_SRC_METHOD == NUX_M1_SRC_IMPL)
        // BOOST (and IMPlicit fallback here): compute xi and use chosen closure
...
# endif
        ...
        (void)compute_Gamma(fidu_w_lorentz[ijk], v_u, Jnew, Enew, Fnew_d, rad_E_floor, rad_eps);
        ...
        if (source_therm_limit < 0 || dt * abs_0[i4D] < source_therm_limit) {
          DrN[ig] = (Nstar + dt * alp[ijk] * volform[ijk] * eta_0[i4D]) /
                    (1 + dt * alp[ijk] * abs_0[i4D] /
                         (fidu_w_lorentz[ijk] > 0 ? fidu_w_lorentz[ijk] : 1.0))
                    - Nstar;
        } else {
          DrN[ig] = (nueave[i4D] > 0 ? (fidu_w_lorentz[ijk] * Jnew) / nueave[i4D] - Nstar : 0.0);
        }
      ...
      if (source_limiter >= 0) {
        ...
        if (DYe > 0) {
          theta = min(theta, source_limiter * max(source_Ye_max - Y_e[ijk], 0.0) / DYe);
        } else if (DYe < 0) {
          theta = min(theta, source_limiter * min(source_Ye_min - Y_e[ijk], 0.0) / DYe);
        }
