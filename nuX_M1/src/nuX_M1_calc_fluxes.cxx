...
template <int dir>
void M1_CalcFlux(CCTK_ARGUMENTS) {
  DECLARE_CCTK_ARGUMENTSX_nuX_Fluxes;
  ...
  //TODO: might have to define the respective three face-centered layouts
  //Below one is a place holder
  const GF3D2layout layout2(cctkGH, {1, 1, 1});

  constexpr array<int, dim> face_centred = {!(dir == 0), !(dir == 1), !(dir == 2)};
    ...
    for (int ig = 0; ig < groupspec; ++ig)
      {
        ...
        // Get the fluid variables from the corresponding cell (for simplicity we use the face point).
        ...
        CCTK_REAL Gamma = compute_Gamma(fidu_w_lorentz[ijk], v_u, rJ[i4D], rE[i4D], F_d);
        CCTK_REAL nnu = rN[i4D] / Gamma;

        //--- Compute the flux components at the face ---
        // Note: dir+1 converts from 0-indexed C++ to the 1-indexed convention used in the physics routines.
        CCTK_REAL flux[5];
        ...
        //--- Store the computed flux into the proper face-centered grid function ---
        // TODO: need to define the following grid functions
        if (dir == 0) {
          for (int iv = 0; iv < 5; ++iv)
            face_flux_x[ PINDEX1D(ig, iv) ][ijk] = flux[iv];
        } else if (dir == 1) {
          for (int iv = 0; iv < 5; ++iv)
            face_flux_y[ PINDEX1D(ig, iv) ][ijk] = flux[iv];
        } else if (dir == 2) {
          for (int iv = 0; iv < 5; ++iv)
            face_flux_z[ PINDEX1D(ig, iv) ][ijk] = flux[iv];
        }
      } // end loop over ig
    } // end lambda
  ); // end grid.loop_int_device for faces
} // end M1_CalcFlux 

//---------------------------------------------------------------------
// (B) M1_UpdateRHSFromFluxes<dir>
// For a given direction, loop over cell centers (centering <1,1,1>) to update the RHS
// using the flux differences computed at cell faces. For each cell the left face (at i_d-1)
// and right face (at i_d) are used. A flux limiter and high-Peclet dissipation fix is applied.
//---------------------------------------------------------------------
template <int dir>
  ...
  grid.loop_int_device<1, 1, 1>(grid.nghostzones,
    [=] CCTK_DEVICE(const Loop::PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE
    {
      ...
      if (dir == 0) {
        ...
        face_left_idx  = layout2.linear(p.i, p.j, p.k);
        face_right_idx = ijk;
      }
      ...
      for (int ig = 0; ig < groupspec; ++ig) {
        for (int iv = 0; iv < 5; ++iv) {
          ...
          if (dir == 0) {
            flux_left  = face_flux_x[ PINDEX1D(ig, iv) ][ face_left_idx ];
            flux_right = face_flux_x[ PINDEX1D(ig, iv) ][ face_right_idx ];
          } else if (dir == 1) {
            flux_left  = face_flux_y[ PINDEX1D(ig, iv) ][ face_left_idx ];
            flux_right = face_flux_y[ PINDEX1D(ig, iv) ][ face_right_idx ];
          } else { // dir == 2
            flux_left  = face_flux_z[ PINDEX1D(ig, iv) ][ face_left_idx ];
            flux_right = face_flux_z[ PINDEX1D(ig, iv) ][ face_right_idx ];
          }

          // --- Optional: Apply flux-limiter/dissipation fix ---
          // For demonstration we recompute a limited flux using neighboring cell-centered conserved data.
          // Here we read four neighboring cell values along the direction:
          // For simplicity we use rN, rFx, etc. (depending on iv) as representative of the conserved variable.
          // In a full implementation, you would select the proper variable depending on iv.
          int shift[3];
          shift[0] = (dir == 0) ? 1 : 0;
          shift[1] = (dir == 1) ? 1 : 0;
          shift[2] = (dir == 2) ? 1 : 0;
          ...
          // Left neighbor:
          idx_m  = layout2.linear(p.i - shift[0], p.j - shift[1], p.k - shift[2]);  
          // Current cell:
          idx    = ijk;
          // Right neighbor:
          idx_p  = layout2.linear(p.i + shift[0], p.j + shift[1], p.k + shift[2]);
          // Next to right:
          idx_pp = layout2.linear(p.i + 2*shift[0], p.j + 2*shift[1], p.k + 2*shift[2]);
          ...
          CCTK_REAL ujm = (iv==0 ? rN[idx_m] : (iv==1 ? rFx[idx_m] : (iv==2 ? rFy[idx_m] : (iv==3 ? rFz[idx_m] : rE[idx_m]))));
          CCTK_REAL uj  = (iv==0 ? rN[idx]   : (iv==1 ? rFx[idx]   : (iv==2 ? rFy[idx]   : (iv==3 ? rFz[idx]   : rE[idx]))));
          CCTK_REAL ujp = (iv==0 ? rN[idx_p] : (iv==1 ? rFx[idx_p] : (iv==2 ? rFy[idx_p] : (iv==3 ? rFz[idx_p] : rE[idx_p]))));
          CCTK_REAL ujpp= (iv==0 ? rN[idx_pp]: (iv==1 ? rFx[idx_pp]: (iv==2 ? rFy[idx_pp]: (iv==3 ? rFz[idx_pp]: rE[idx_pp]))));

          // Compute dissipation factor from nearby "abs" and "scat" values.
          CCTK_REAL kapp = 0.5 * ( abs_1[idx] + abs_1[idx_p] + scat_1[idx] + scat_1[idx_p] );
          CCTK_REAL A = 1.0;
          if (kapp * delta[dir] > 1.0) {
              A = std::min(1.0, 1.0 / (delta[dir] * kapp));
              A = std::max(A, mindiss);
          }
          ...
          CCTK_REAL cmx = std::max(std::abs(flux_left), std::abs(flux_right));
          CCTK_REAL flux_low  = 0.5 * (flux_left + flux_right - cmx * (ujp - uj));
          CCTK_REAL flux_high = 0.5 * (flux_left + flux_right);
          CCTK_REAL flux_num  = flux_high - (sawtooth ? 1.0 : A) * (1 - phi) * (flux_high - flux_low);
...
// It launches the face–flux kernel and the cell–centered RHS update kernel for each spatial direction.
//---------------------------------------------------------------------
extern "C" void nuX_M1_CalcFluxes(CCTK_ARGUMENTS)
{
  ...
  // For each spatial direction, first compute the face fluxes,
  // then update the cell-centered RHS using those fluxes.

  M1_CalcFlux<0>(CCTK_PASS_ARGUMENTS);
  M1_UpdateRHSFromFluxes<0>(CCTK_PASS_ARGUMENTS);

  M1_CalcFlux<1>(CCTK_PASS_ARGUMENTS);
  M1_UpdateRHSFromFluxes<1>(CCTK_PASS_ARGUMENTS);

  M1_CalcFlux<2>(CCTK_PASS_ARGUMENTS);
  M1_UpdateRHSFromFluxes<2>(CCTK_PASS_ARGUMENTS);
}
