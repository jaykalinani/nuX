...
CCTK_DEVICE CCTK_REAL volume(CCTK_REAL R, CCTK_REAL xp, CCTK_REAL yp, CCTK_REAL zp,
  ...
  for (int ii = 0; ii < NPOINTS; ++ii) {
    ...
    for (int jj = 0; jj < NPOINTS; ++jj) {
      ...
      for (int kk = 0; kk < NPOINTS; ++kk) {
        ...
        ++count;
        if (myx*myx + myy*myy + myz*myz < R*R) ++inside;
      }
    }
  }
...
extern "C" void nuX_M1_SetupTest(CCTK_ARGUMENTS) {
  ...
  if (verbose) CCTK_INFO("nuX_M1_SetupTest");
      ...
      if (CCTK_Equals(thc_m1_test, "beam")) {
        ...
        for (int ig = 0; ig < ngroups*nspecies; ++ig) {
          ...
          if (proj < beam_position && offset2 < beam_width*beam_width) {
            rE[i4D] = 1.0; rN[i4D] = 1.0;
            rFx[i4D] = nx; rFy[i4D] = ny; rFz[i4D] = nz;
          } else {
            rE[i4D] = 0.0; rN[i4D] = 0.0;
            rFx[i4D] = 0.0; rFy[i4D] = 0.0; rFz[i4D] = 0.0;
          }
        }
      }
...
extern "C" void nuX_M1_KerrSchild_Mask(CCTK_ARGUMENTS) {
  ...
  if (verbose) CCTK_INFO("nuX_M1_KerrSchild_Mask");
  ...
  grid.loop_all_device<1,1,1>(
    ...
    [=] CCTK_DEVICE(const PointDesc &p) {
      const int ijk = layout2.linear(p.i, p.j, p.k);
      if (x[ijk]*x[ijk] + y[ijk]*y[ijk] + z[ijk]*z[ijk] < kerr_mask_radius*kerr_mask_radius) {
        nuX_m1_mask[ijk] = 1;
...
extern "C" void nuX_M1_SetupTest_Hydro(CCTK_ARGUMENTS) {
  ...
  if (verbose) CCTK_INFO("nuX_M1_SetupTest_Hydro");
