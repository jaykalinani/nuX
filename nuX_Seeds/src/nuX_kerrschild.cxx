  ...
  grid.loop_int_device<1, 1, 1>(
      ...
      [=] CCTK_DEVICE(const PointDesc &p) CCTK_ATTRIBUTE_ALWAYS_INLINE {
        ...
        if (in_beam) {
          ...
          E = sqrt(fmax(detg, 0.0));

          CCTK_REAL const inv_alp = (alp_ijk > 0.0) ? (1.0 / alp_ijk) : 0.0;
          CCTK_REAL const F_u_x = (a + beta_u_x) * E * inv_alp;
          CCTK_REAL const F_u_y = beta_u_y * E * inv_alp;
          CCTK_REAL const F_u_z = beta_u_z * E * inv_alp;

          F_d_x = g_xx * F_u_x + g_xy * F_u_y + g_xz * F_u_z;
          F_d_y = g_xy * F_u_x + g_yy * F_u_y + g_yz * F_u_z;
          F_d_z = g_xz * F_u_x + g_yz * F_u_y + g_zz * F_u_z;
          N = 1.0;
        }
